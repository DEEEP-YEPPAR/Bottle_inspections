...
# Initialize YOLO model with safe loading
@st.cache_resource
def load_model():
    try:
        # Add safe globals to allow loading our model
        from torch.serialization import _get_restore_location, _maybe_decode_ascii, _safe_globals
        _safe_globals.add('ultralytics.nn.tasks.DetectionModel')
        
        # Load model with weights_only=False for custom model support
        model = torch.load(
            "Weights//Weights//3000_best_60single.pt", 
            map_location='cpu',
            weights_only=False
        )
        
        # For YOLOv8 models, we need to wrap the loaded model
        if isinstance(model, dict) and 'model' in model:
            from ultralytics.nn.tasks import DetectionModel
            model = DetectionModel(model['model'].yaml)
            model.load_state_dict(model['model'].state_dict())
